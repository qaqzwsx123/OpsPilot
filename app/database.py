...
def seed_knowledge_library() -> None:
    ...
    documents = [
        ...
        ("P1 告警处置 SOP", "1. 5 分钟内确认告警范围、影响设备和最近变更。\n2. 依次检查网络、心跳、供电和网关日志。\n3. 未恢复时创建高优工单并升级值班负责人。\n4. 关闭前记录根因、处理动作和验证结果。", "P1,告警,SOP,升级,复盘"),
        ("受控数据库查询规范", "只生成单条 SELECT；只访问已授权的数据表和字段；查询必须限制返回行数，默认最多 100 行。需要修改数据时创建审批单，不直接执行写操作。", "SQL,查询,只读,权限,审批,规范"),
    ]
    with connect() as conn:
